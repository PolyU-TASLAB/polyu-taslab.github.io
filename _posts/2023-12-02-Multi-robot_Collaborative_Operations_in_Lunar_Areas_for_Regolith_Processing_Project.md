...
********************************
Research on lunar surface requires Unmanned Autonomous Systems (UASs) with robotic arms for soil sample collection. Challenges include precise control on rugged terrain and ensuring safety. A multi-robot collaboration system is proposed to address these challenges, using Factor Graph Optimization (FGO) for accurate positioning. The system achieves a RMSE of 0.30 meters, a significant improvement over existing methods. The Leader-Follower Formation Algorithm is utilized for efficient multi-robot coordination. Future work will focus on enhancing collaboration through Positioning and Leader-Follower algorithms for autonomous navigation and soil sampling. The system incorporates automated navigation, soil identification, and collection using QR codes for precise localization and task assignment.
